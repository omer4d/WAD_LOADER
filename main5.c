...
void render_col_helper(int x, float y0, float y1, float z, int c)
{
    float h1 = SCREEN_H / 2 - y0 * FOCAL_DIST / z;
    float h2 = SCREEN_H / 2 - y1 * FOCAL_DIST / z;
    rectfill(buffer, x * 2, (int)h1, x * 2 + 2, (int)h2, c);
}
    ...
    if(idx & SHORT_SIGN_FLAG)
    {
        ...
        for(i = 0; i < ssector->seg_num; ++i)
        {
            ...
            if(res.t > 0 && res.k >= 0 && res.k <= 1.0)
            {
                ...
                int color = colors[ssector->first_seg_idx + i];//makecol(r, g, 0);
                ...
                if(sidedef1 && !sidedef2) {
                    ...
                    float top_h = MAX(SCREEN_H / 2 - top_y * FOCAL_DIST / z, *minh);
                    float bottom_h = MIN(SCREEN_H / 2 - bottom_y * FOCAL_DIST / z, *maxh);
                                //printf("%d ", ssector->first_seg_idx + i);
                    
                    if(bottom_h > top_h && vec2f_dot(n, vec2f_diff(vp, v1)) < 0)
                ...
                {   
                    ...
                    if(h2 > h1 && vec2f_dot(n, vec2f_diff(vp, v1)) < 0)
...
int main()
{
    ...
    int i;    
    for(i = 0; i < 1000; ++i)
      colors[i] = makecol(rand() % 256, rand() % 256, rand() % 256);
